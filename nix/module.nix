...
let
  ...
  tcpPorts = with cfg; [ http.port ]
    ++ optional (https.enable && https.redirect.enable) https.redirect.port
...
in
{
  config = mkIf cfg.enable {
    assertions = [
      ...
      {
        assertion = (cfg.https.enable && cfg.https.redirect.enable) -> (cfg.http.port == 443);
        message = ''
          `services.broadcast-box.https.redirect.enable` only works if
          `http.port` is 443. Either disable redirect or change the port.
        '';
      }
      {
    ...
    systemd.services.broadcast-box = {
      ...
      environment = with cfg; {
        ...
        ENABLE_HTTP_REDIRECT = mkIf (https.enable && https.redirect.enable) "true";
        HTTPS_REDIRECT_PORT = mkIf (https.enable && https.redirect.enable) (toString https.redirect.port);
  ...
  options.services.broadcast-box = {
    ...
    https.redirect.enable = mkEnableOption ''
      an HTTP server on port `https.redirect.port` that redirects HTTP traffic
      to HTTPS.

      Only works if `http.port` is 443.
    '';

    https.redirect.port = mkOption {
      type = types.port;
      default = 80;
      description = ''
        TCP port the HTTPS redirect server will listen on.
      '';
    };
